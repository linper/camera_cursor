...
mix_rgb_palette = np.array([[84, 65, 76], [7, 7, 7], [96, 18, 18], [200, 100, 93], [58, 39, 48], [38, 11, 16], [163, 51, 41]])
...
def proc_image(file, model=None, is_file=True, show=False):
    ...
    for i, kpt in enumerate(kpts):
        ...
        kpt_bb = (int(kpt.pt[0]-kpt.size*0.7), int(kpt.pt[1]-kpt.size*0.7), int(1.4*kpt.size), int(1.4*kpt.size))
...
def downscale_image(file):
    ...
    for i in [1,2,4,8,16]:
        ...
        downscale_factor = cv2.resize(image, (original_width // i, original_height // i), interpolation=cv2.INTER_AREA)

        # Step 4: Display the downscaled images
        plt.imshow(downscale_factor)
        raw_name = os.path.splitext(os.path.basename(file))[0]  # Returns 'example_image'
        plt.tight_layout()
        cv2.imwrite(f"img/test_down/{raw_name}_{i}.png", cv2.cvtColor(downscale_factor, cv2.COLOR_RGB2BGR))
